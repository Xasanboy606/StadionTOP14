...
db_1.pool.query(`ALTER TABLE events ADD COLUMN IF NOT EXISTS image_url TEXT`).catch(() => { });
...
db_1.pool.query(`SELECT count(*) FROM events`).then(async ({ rows }) => {
...
router.get("/", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const { rows } = await db_1.pool.query(`SELECT e.*,
        COALESCE(
...
router.get("/public", async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query(`SELECT * FROM events WHERE is_active = TRUE ORDER BY event_date ASC`);
        res.json(rows);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.post("/:id/book", auth_1.authenticate, async (req, res) => {
    const { seats } = req.body;
    const userId = req.user.id;
    const eventId = req.params.id;
    if (!seats || seats < 1)
        return res.status(400).json({ error: "Noto'g'ri o'rindiq soni" });
    try {
        const { rows: ev } = await db_1.pool.query("SELECT * FROM events WHERE id=$1 AND is_active=TRUE", [eventId]);
        if (!ev.length)
            return res.status(404).json({ error: "Tadbir topilmadi" });
        const event = ev[0];
        const available = event.capacity - event.taken;
        if (seats > available)
            return res.status(400).json({ error: "Yetarli o'rin yo'q" });
        const total = seats * event.price_per_seat;
        /* Upsert booking */
        const { rows: existing } = await db_1.pool.query("SELECT id, seats FROM event_bookings WHERE event_id=$1 AND user_id=$2", [eventId, userId]);
        let finalSeats = seats;
        if (existing.length) {
            const diff = seats - existing[0].seats;
            if (event.taken + diff > event.capacity)
                return res.status(400).json({ error: "Yetarli o'rin yo'q" });
            await db_1.pool.query("UPDATE event_bookings SET seats=$1, total=$2 WHERE event_id=$3 AND user_id=$4", [seats, total, eventId, userId]);
            await db_1.pool.query("UPDATE events SET taken = taken + $1 WHERE id=$2", [diff, eventId]);
        }
        else {
            await db_1.pool.query("INSERT INTO event_bookings (event_id, user_id, seats, total) VALUES ($1,$2,$3,$4)", [eventId, userId, seats, total]);
            await db_1.pool.query("UPDATE events SET taken = taken + $1 WHERE id=$2", [seats, eventId]);
        }
        const { rows: updated } = await db_1.pool.query("SELECT * FROM events WHERE id=$1", [eventId]);
...
router.post("/admin", auth_1.authenticate, async (req, res) => {
    if (!req.user.roles?.includes("admin"))
        return res.status(403).json({ error: "Ruxsat yo'q" });
    ...
    try {
        const { rows } = await db_1.pool.query(`INSERT INTO events (league, title, home_team, away_team, event_date, venue, district,
...
router.patch("/admin/:id", auth_1.authenticate, async (req, res) => {
    if (!req.user.roles?.includes("admin"))
        return res.status(403).json({ error: "Ruxsat yo'q" });
    const { id } = req.params;
    const { league, title, home_team, away_team, event_date, venue, district, price_per_seat, capacity, accent, emoji, image_url, is_active } = req.body;
...
router.delete("/admin/:id", auth_1.authenticate, async (req, res) => {
    if (!req.user.roles?.includes("admin"))
        return res.status(403).json({ error: "Ruxsat yo'q" });
    try {
...
router.get("/my-bookings", auth_1.authenticate, async (req, res) => {
    try {
        const { rows } = await db_1.pool.query(`SELECT eb.*, e.title, e.league, e.home_team, e.away_team, e.event_date, e.venue, e.district, e.price_per_seat
       FROM event_bookings eb
       JOIN events e ON eb.event_id = e.id
       WHERE eb.user_id = $1
       ORDER BY eb.created_at DESC`, [req.user.id]);
        res.json(rows);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
