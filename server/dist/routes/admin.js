...
router.get("/stats", ...guard, async (_req, res) => {
    try {
        const [users, stadiums, bookings, rev] = await Promise.all([
            ...
            db_1.pool.query("SELECT COUNT(*) FROM stadiums"),
            db_1.pool.query("SELECT COUNT(*) FROM bookings"),
            db_1.pool.query("SELECT COALESCE(SUM(paid_amount),0) as total FROM bookings WHERE status='confirmed'"),
        ]);
        res.json({
...
router.get("/stadiums", ...guard, async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query("SELECT * FROM stadiums ORDER BY created_at DESC");
        res.json(rows);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.patch("/stadiums/:id", ...guard, async (req, res) => {
    const { name, district, address, description, size, price_day, price_night, facilities, has_referee, has_video, has_balls, has_bibs, lat, lng, images, status, } = req.body;
    try {
        const { rows: [stadium] } = await db_1.pool.query(`UPDATE stadiums SET
       ...
       RETURNING *`, [
        ...
        if (!stadium)
            return res.status(404).json({ error: "Stadion topilmadi" });
        res.json(stadium);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.patch("/stadiums/:id/status", ...guard, async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query("UPDATE stadiums SET status=$1, updated_at=NOW() WHERE id=$2 RETURNING *", [status, req.params.id]);
        if (!rows.length)
        ...
        await db_1.pool.query(`INSERT INTO notifications (user_id, title, body, type, meta) VALUES ($1,$2,$3,'admin',$4)`, [stadium.owner_id, notifTitle, notifBody, JSON.stringify({ stadium_id: stadium.id, status })]).catch(() => { });
        res.json(stadium);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.get("/edit-requests", ...guard, async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query(`SELECT er.*, s.name as stadium_name, p.full_name
...
router.patch("/edit-requests/:id", ...guard, async (req, res) => {
    ...
    try {
        ...
        await client.query("UPDATE edit_requests SET status=$1, admin_response=$2, reviewed_at=NOW() WHERE id=$3", [action, admin_response || null, req.params.id]);
        if (action === "approved") {
            ...
            if (allowed.includes(er.field_name)) {
                let val = er.new_value;
                if (numFields.includes(er.field_name))
                    val = parseInt(er.new_value);
                if (boolFields.includes(er.field_name))
                    val = er.new_value === "true";
                if (jsonFields.includes(er.field_name) || er.field_name === "images") {
                    try {
                        val = JSON.parse(er.new_value);
                    }
                    catch {
                        val = er.new_value;
                    }
                }
                await client.query(`UPDATE stadiums SET ${er.field_name}=$1, updated_at=NOW() WHERE id=$2`, [val, er.stadium_id]);
            }
            /* Notify owner that edit request was approved */
            await client.query(`INSERT INTO notifications (user_id, title, body, type, meta)
...
router.get("/profiles", ...guard, async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query(`SELECT u.id, u.email, u.created_at, p.full_name, p.phone, p.is_blocked,
...
router.patch("/profiles/:id/block", ...guard, async (req, res) => {
    ...
    try {
        await db_1.pool.query("UPDATE profiles SET is_blocked=$1 WHERE id=$2", [!!is_blocked, req.params.id]);
        res.json({ ok: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.patch("/profiles/:id/role", ...guard, async (req, res) => {
    ...
    try {
        if (action === "add") {
            await db_1.pool.query("INSERT INTO user_roles (user_id, role) VALUES ($1, $2) ON CONFLICT DO NOTHING", [req.params.id, role]);
        }
        else {
            await db_1.pool.query("DELETE FROM user_roles WHERE user_id=$1 AND role=$2", [req.params.id, role]);
        }
        res.json({ ok: true });
    }
    catch (e) {
...
router.get("/transactions", ...guard, async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query(`SELECT t.*, b.short_code, b.booking_date
...
router.patch("/transactions/:id/release", ...guard, async (req, res) => {
    try {
        await db_1.pool.query("UPDATE transactions SET escrow_status='released', released_at=NOW() WHERE id=$1", [req.params.id]);
        res.json({ ok: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.post("/stadiums", ...guard, async (req, res) => {
    const { name, district, address, description, size, price_day, price_night, facilities, has_referee, has_video, has_balls, has_bibs, lat, lng, images, } = req.body;
    ...
    try {
        const { rows: [stadium] } = await db_1.pool.query(`INSERT INTO stadiums
...
router.delete("/stadiums/:id", ...guard, async (req, res) => {
    try {
        await db_1.pool.query("DELETE FROM stadiums WHERE id=$1", [req.params.id]);
        res.json({ ok: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.get("/stadiums/:id/stats", ...guard, async (req, res) => {
    const today = new Date().toISOString().slice(0, 10);
    try {
        const { rows: [r] } = await db_1.pool.query(`
      SELECT
...
router.get("/stadiums/:id/bookings", ...guard, async (req, res) => {
    const { from, to, payment_status, booking_status } = req.query;
    const conditions = ["b.stadium_id = $1"];
    const params = [req.params.id];
    let p = 2;
    if (from) {
        conditions.push(`b.booking_date >= $${p++}`);
        params.push(from);
    }
    if (to) {
        conditions.push(`b.booking_date <= $${p++}`);
    ...
    if (payment_status && payment_status !== "all") {
        conditions.push(`b.payment_kind = $${p++}`);
...
router.get("/stadiums/:id/financial", ...guard, async (req, res) => {
    ...
    try {
        const { rows: [fin] } = await db_1.pool.query(`
      SELECT
    ...
    `, params);
        ...
        const { rows: [tx] } = await db_1.pool.query(`SELECT COALESCE(SUM(amount), 0)::INTEGER AS transferred FROM transfers WHERE stadium_id = $1`, [req.params.id]);
...
router.post("/stadiums/:id/transfers", ...guard, async (req, res) => {
    ...
    try {
        const { rows: [st] } = await db_1.pool.query("SELECT owner_id FROM stadiums WHERE id=$1", [req.params.id]);
        if (!st)
            return res.status(404).json({ error: "Stadion topilmadi" });
        const { rows: [tr] } = await db_1.pool.query(`INSERT INTO transfers (stadium_id, owner_id, admin_id, amount, note)
       VALUES ($1,$2,$3,$4,$5) RETURNING *`, [req.params.id, st.owner_id, req.user.id, parseInt(amount), note || null]);
        res.json(tr);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.patch("/stadiums/:id/assign-owner", ...guard, async (req, res) => {
    ...
    if (!owner_id)
        return res.status(400).json({ error: "owner_id kerak" });
    try {
        const { rows: [role] } = await db_1.pool.query("SELECT id FROM user_roles WHERE user_id=$1 AND role='owner'", [owner_id]);
        if (!role)
            return res.status(400).json({ error: "Bu foydalanuvchi owner roliga ega emas" });
        await db_1.pool.query("UPDATE stadiums SET owner_id=$1, updated_at=NOW() WHERE id=$2", [owner_id, req.params.id]);
        const { rows: [st] } = await db_1.pool.query("SELECT name FROM stadiums WHERE id=$1", [req.params.id]);
        await db_1.pool.query(`INSERT INTO notifications (user_id,title,body,type,meta) VALUES ($1,$2,$3,'admin',$4)`, [owner_id, "🏟️ Stadion biriktirildi!", `Sizga "${st?.name}" stadioni biriktirildi.`,
            JSON.stringify({ stadium_id: req.params.id })]).catch(() => { });
        res.json({ ok: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.get("/events", ...guard, async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query("SELECT * FROM events ORDER BY created_at DESC");
        res.json(rows);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.post("/events", ...guard, async (req, res) => {
    const { league, title, home_team, away_team, event_date, venue, district, price_per_seat, capacity, accent, emoji, image_url, is_active } = req.body;
    if (!league || !title || !event_date || !venue) {
        return res.status(400).json({ error: "Majburiy maydonlar: league, title, event_date, venue" });
    }
    try {
        const { rows } = await db_1.pool.query(`INSERT INTO events
        (league, title, home_team, away_team, event_date, venue, district,
         price_per_seat, capacity, taken, accent, emoji, image_url, is_active)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,0,$10,$11,$12,$13) RETURNING *`, [league, title, home_team || "", away_team || "", event_date, venue, district || "",
...
router.patch("/events/:id", ...guard, async (req, res) => {
    ...
    const { league, title, home_team, away_team, event_date, venue, district, price_per_seat, capacity, accent, emoji, image_url, is_active } = req.body;
    try {
        const { rows } = await db_1.pool.query(`UPDATE events SET
       ...
       WHERE id=$14 RETURNING *`, [league || "", title || "", home_team || "", away_team || "",
...
router.delete("/events/:id", ...guard, async (req, res) => {
    try {
        await db_1.pool.query("DELETE FROM events WHERE id=$1", [req.params.id]);
        res.json({ ok: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
