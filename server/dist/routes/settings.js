...
router.get("/", async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query("SELECT data FROM site_settings WHERE id=1");
        const stored = rows[0]?.data ?? {};
        res.json(deepMerge(exports.DEFAULT_SETTINGS, stored));
    }
    catch (e) {
...
router.patch("/", auth_1.authenticate, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        const { rows } = await db_1.pool.query(`INSERT INTO site_settings (id, data, updated_at) VALUES (1, $1, NOW())
       ON CONFLICT (id) DO UPDATE SET data=$1, updated_at=NOW()
       RETURNING data`, [JSON.stringify(req.body)]);
