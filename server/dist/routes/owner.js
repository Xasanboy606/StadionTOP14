...
const guard = [auth_1.authenticate, (0, auth_1.requireRole)("owner")];
...
router.get("/stadiums", ...guard, async (req, res) => {
    try {
        const { rows } = await db_1.pool.query("SELECT * FROM stadiums WHERE owner_id=$1 ORDER BY created_at DESC", [req.user.id]);
        res.json(rows);
    }
    catch (e) {
...
router.get("/bookings", ...guard, async (req, res) => {
    try {
        const { rows: stadiums } = await db_1.pool.query("SELECT id FROM stadiums WHERE owner_id=$1", [req.user.id]);
        if (!stadiums.length)
            return res.json([]);
        const ids = stadiums.map((s) => s.id);
        const { rows } = await db_1.pool.query(`SELECT b.*, s.name as stadium_name, p.full_name as customer_name, p.phone as customer_phone
       FROM bookings b
       LEFT JOIN stadiums s ON s.id=b.stadium_id
       LEFT JOIN profiles p ON p.id=b.user_id
       WHERE b.stadium_id = ANY($1::uuid[])
       ORDER BY b.booking_date DESC, b.hour DESC`, [ids]);
...
router.get("/closed-slots", ...guard, async (req, res) => {
    const { stadium_id, date } = req.query;
    if (!stadium_id || !date)
        return res.json([]);
    try {
        /* Verify ownership */
        const { rows: own } = await db_1.pool.query("SELECT id FROM stadiums WHERE id=$1 AND owner_id=$2", [stadium_id, req.user.id]);
        if (!own.length)
            return res.status(403).json({ error: "Ruxsat yo'q" });
        const { rows } = await db_1.pool.query("SELECT hour FROM closed_slots WHERE stadium_id=$1 AND slot_date=$2 ORDER BY hour", [stadium_id, date]);
        res.json(rows.map((r) => r.hour));
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post("/closed-slots/toggle", ...guard, async (req, res) => {
    ...
    try {
        ...
        const { rows: existing } = await db_1.pool.query("SELECT id FROM closed_slots WHERE stadium_id=$1 AND slot_date=$2 AND hour=$3", [stadium_id, date, hour]);
        if (existing.length) {
            await db_1.pool.query("DELETE FROM closed_slots WHERE stadium_id=$1 AND slot_date=$2 AND hour=$3", [stadium_id, date, hour]);
            res.json({ closed: false });
        }
        else {
            await db_1.pool.query("INSERT INTO closed_slots (stadium_id, slot_date, hour) VALUES ($1,$2,$3) ON CONFLICT DO NOTHING", [stadium_id, date, hour]);
...
router.get("/range-stats", ...guard, async (req, res) => {
    ...
    try {
        ...
        const { rows: [s] } = await db_1.pool.query(`
      SELECT
