...
router.get("/", async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query("SELECT * FROM stadiums WHERE status='approved' ORDER BY created_at DESC");
        res.json(rows);
    }
    catch (e) {
...
router.get("/booked-hours", async (req, res) => {
    ...
    if (!date)
        return res.json({});
    try {
        const { rows } = await db_1.pool.query("SELECT stadium_id, hour, duration FROM bookings WHERE booking_date=$1 AND status='confirmed'", [date]);
        const map = {};
        for (const r of rows) {
            if (!map[r.stadium_id])
                map[r.stadium_id] = [];
...
router.get("/:id", async (req, res) => {
    try {
        const { rows } = await db_1.pool.query("SELECT * FROM stadiums WHERE id=$1", [req.params.id]);
        if (!rows.length)
...
router.get("/:id/slots", async (req, res) => {
    ...
    if (!date)
        return res.json([]);
    try {
        const { rows } = await db_1.pool.query("SELECT hour, duration FROM bookings WHERE stadium_id=$1 AND booking_date=$2 AND status='confirmed'", [req.params.id, date]);
        res.json(rows);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.post("/", auth_1.authenticate, (0, auth_1.requireRole)("owner"), async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query(`INSERT INTO stadiums
...
router.patch("/:id", auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        const { rows } = await db_1.pool.query(`UPDATE stadiums SET ${sets}, updated_at=NOW() WHERE id=$1 RETURNING *`, vals);
        res.json(rows[0]);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
